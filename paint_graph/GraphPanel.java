...
public class GraphPanel extends JPanel  {
    ...
    public void paint(Graphics g) {
        ...
        addText(g, "Граф", 30,getWidth() / 2, 20);
    ...
    public void changeColorOfEdge(int node1, int node2)
    {
        Color green = new Color(0,255,0);
        changeColorOfNode(node1);
        changeColorOfNode(node2);
        for (int i = 0; i < edges.length; i++){
            if (edges[i].getDestination().equals(nodes[node1]) && edges[i].getSource().equals(nodes[node2])) {
                edges[i].setColorEdge(green);
                return;
            }
